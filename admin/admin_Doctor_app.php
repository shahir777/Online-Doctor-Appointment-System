...
{ 
  ...
  require('./comnpages/config.php');
    ...
    $sql="SELECT * FROM appoinment where status = 0  ";
             ...
             <header class="panel-heading">
              ...
              -->
                          ...
                          <table class="table table-striped table-advance table-hover">
                           <tbody>
                              <tr>
                               <th><i class="icon_profile"></i>app_token</th>
    ...
    if ($result) {
        if (mysqli_num_rows($result) > 0) {
            while ($user = mysqli_fetch_array($result)) {  
                $c_id = $user['app_id'];
                $did = $user['app_doctor'];
                $select = "SELECT * FROM doctor_list WHERE `did` = '$did'";
                $res = mysqli_query($conn, $select);
                if ($res && mysqli_num_rows($res) > 0) {
                    $doctor = mysqli_fetch_array($res);
...
?>
          ...
          <button name="view" title="Visited" class="btn btn-primary">
